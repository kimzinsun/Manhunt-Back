...
fun main() {
    val filterService = BadWordFilterService()

    val testText = "씨 발 테스트 씨발문자열에 비속씨 발어가 ㅅㅂ 포함되어 화염병 ㅅ ㅂ 있다면 씨@!@#발 이를 확인하십시오씨발 ㅅㅂ ㅅ123ㅂ 씨@발."
    // '시ㅋㅋ발'과 같은 문자는 필터링되지 않음

    println("Bad words check: ${filterService.hasBadWords(testText)}")
    println("Replace bad words: ${filterService.filterBadWords(testText)}")
}
